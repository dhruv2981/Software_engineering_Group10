...
const TextEditor = ({
  ...
  const [triggerHeadingUpdate, resFromHeadingUpdate] =
    documentApi.endpoints.updateHeading.useLazyQuery();
  const [triggerCoverUpdate, resFromCoverUpdate] =
    documentApi.endpoints.updateCover.useLazyQuery();
  const [triggerDocumentUpdate, res] =
    documentApi.endpoints.updateDocCotent.useLazyQuery();
  const [triggerTextUpdate, resFromTextUpdate] =
    documentApi.endpoints.updateDocTextContent.useLazyQuery();
  ...
  useEffect(() => {
    if (data?.doc) editorRef.current?.commands.setContent(data.doc);
  }, [data]);

  const headingCLass =
    "px-1 min-w-4/5 max-w-4/5 mx-auto focus-visible:outline-none text-5xl font-bold font-sans " +
    (cover || data?.meta.cover ? "mt-10" : "mt-[120px]");
    ...
    <div className="w-full">
      {error ? (
        ...
        <>
          {data?.meta.cover ? (
            <img
              src={"http://localhost:8000" + data?.meta.cover}
          ...
          >
            ...
            {cover || data?.meta?.cover ? (
              <div
                className="p-2 rounded-md flex text-[--border-color] hover:text-zinc-700 hover:bg-zinc-300 hover:cursor-pointer w-min space-x-1"
                onClick={removeCover}
              >
